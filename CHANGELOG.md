...
- **One-click GLM Vision MCP server** - install the official Z.AI vision MCP server (`@z_ai/mcp-server`, GLM-4.6V) with **GLM: Install GLM Vision MCP Server** or the new walkthrough step. Nothing is registered silently: the flow asks for your API key (contextualized prompt), checks for Node.js 18+ (22+ recommended), and only then adds **GLM Vision** to VS Code's MCP list, with `Z_AI_MODE` set from your region (`ZAI` International, `ZHIPU` Mainland China). Your stored API key (`SecretStorage`, or the settings fallback) is injected only when VS Code starts the server; if it is missing or the prompt is cancelled, an actionable warning points back to **GLM: Set API Key**. Region or key changes while the server is running prompt you to restart it from the MCP list. **GLM: Uninstall GLM Vision MCP Server** removes the server and cleans up.
- **Image input for text-only GLM models** - once GLM Vision is installed and running, **GLM: Toggle Vision for Chat Models** (or `glm-copilot.visionEnabled`) turns on image input: every GLM model in the picker then accepts pasted or attached images. Each image is analyzed by the GLM Vision server (`analyze_image`, GLM-4.6V) and injected as text before the request reaches the chat model — visible code, terminal output, errors, and UI included. The analysis instruction is configurable via `glm-copilot.visionPrompt` (empty = built-in prompt; changing it re-analyzes images on the next turn). Analyses are content-addressed and cached in memory for the session (never written to extension storage). Images are validated against a type allowlist (`png`/`jpeg`/`webp`/`gif`) and a 5 MB cap before any tool call. A status line streams into the thinking block while analysis runs (cache hits stay silent); if analysis fails or the server stops, models revert to text-only and the reply opens with a localized "image analysis failed" notice instead of erroring out. Note: the earlier `glm-copilot.visionModel` setting and the custom-models `vision` flag are gone — the vision model is fixed server-side (GLM-4.6V).
