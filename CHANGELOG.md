...
- **Image input for text-only GLM models** - paste or attach a screenshot and GLM can now reason about it. Any request that contains an image is routed through a **vision describer proxy**: a house GLM vision model (`glm-4.6v` by default) transcribes each image to text, and that text is injected into the normal chat request so the selected text model gets the content — visible code, terminal output, errors, and UI included. All images in one container (a single message, or a single tool result) are described in one batched call. Descriptions are content-addressed and cached in memory and across window reloads, so conversation history is not re-described every turn; changing the model or prompt invalidates the cache automatically. Configure with the new `glm-copilot.visionModel` (empty = `glm-4.6v`; `glm-4.6v-flash` for lower cost/latency) and `glm-copilot.visionPrompt` (empty = built-in prompt) settings. Before any network call, images are validated against a type allowlist (`png`/`jpeg`/`webp`/`gif`) and a 10 MB cap and rejected with a clear notice otherwise. A status line streams into the thinking block while a real describe call runs (cache hits stay silent); if description fails, the reply continues without the image and opens with a localized "image analysis failed" notice. Image bytes are sent only to your configured GLM endpoint and are never logged.
- **One-click GLM Vision MCP server** - the official Z.AI vision MCP server (`@z_ai/mcp-server`) is now registered automatically for agent mode and appears in VS Code's MCP list as **GLM Vision** with zero configuration. It launches via `npx` (requires Node.js 22+) with `Z_AI_MODE` set from your region (`ZAI` International, `ZHIPU` Mainland China); your API key is injected from `SecretStorage` only when VS Code starts the server, and you are prompted once if none is stored.
